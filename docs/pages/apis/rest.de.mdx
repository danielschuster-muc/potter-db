...
IDs müssen als UUIDs oder Kennung (slugs) angegeben werden.
...
Du kannst unser aktuelles OAS document [hier](https://api.potterdb.com/v1/openapi.json) finden.
...
- **Ressourcen Elemente**: Jede Ressource ist ein Objekt mit einem Typ (`type`), ID (`id`) und Attributen (`attributes`).
- **Verbidungen*: Ressourcen können mit anderen Ressourcen verbunden sein, z. B. ist eine `Buch`-Ressource mit einer `Kapitel`-Ressource verbunden und umgekehrt.
- **Paginierung**: Wenn eine Antwort eine große Anzahl von Ressourcen enthält, wird die Antwort paginiert. Siehe [Paginierung](#pagination) für weiter Infos.
- **Fehler**: Fehler werden mit standardisierten Strukturen zurückgegeben, was die Behandlung und Behebung von Problemen erleichtert.

## Abfragen machen

Um eine Abfrage an die REST-API zu machen, musst du eine "GET"-Anfrage an einen unserer verfügbaren Endpunkte stellen.
...
Du kannst Ressourcen sortieren, indem du einen `sort` Abfrageparameter zu deiner Anfrage hinzufügst. Um beispielsweise Figuren nach Namen zu sortieren, kannst das Attribut `name` verwenden (unter [Ressourcen](#Ressourcen) findest du alle verfügbaren Attribute):
