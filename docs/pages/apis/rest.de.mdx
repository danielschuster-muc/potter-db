...
Du kannst Ressourcen sortieren, indem du einen `sort` Abfrageparameter zu deiner Anfrage hinzufügst. Um beispielsweise Figuren nach Namen zu sortieren, kannst das Attribut `name` verwenden (unter [Ressourcen](/resourcen) findest du alle verfügbaren Attribute):
