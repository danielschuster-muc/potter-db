...
```
Dans cette requête, nous demandons le titre (`title`) et la date de parution (`releaseDate`) des trois premiers livres, ainsi que le titre (`title`) et le `slug` de tous les chapitres de chaque livre. Nous demandons également le `nom` du personnage avec le slug `harry-potter`.
