...
```
In dieser Abfrage werden der Titel (`title`) und das Erscheinungsdatum (`releaseDate`) der ersten drei Bücher, sowie der Titel (`title`) und die Kennung (`lug`) aller Kapitel jedes Buches abgefragt. Außerdem wird der Name (`name`) der Figur mit der Kennung (`slug`) "harry-potter" abgefragt.
