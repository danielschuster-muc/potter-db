...
l'API "Potter DB" est l'interface REST et basée sur GraphQL d'un vaste répertoire de données sur le monde magique de Harry Potter. Nous nous occupons des données, vous avez juste à vous concentrer sur la construction de votre app. Avec cette API vous pouvez :
...
- Obtenir des données détaillés sur les `livres` et les `films` de l’univers Harry Potter.
